...
const NGReturnListIndex = () => {
	...
	let searchClick =async () => {
		...
		setLoading(true);
		let object = {
			url: ApiPath.NGReturnListSearch,
			method: 'get',
			params: {
				"product_code": code,
				"product_name": name,
	...
	let tempSearchClick =async () => {
		...
		if (response.flag === false) {
			...
			if (response.data.status === 'OK') {
				setUserData(response.data.data);setCopyData(response.data.data);setLoading(false);setDeletedID("");
			} else {
				window.scrollTo({ top: 0, left: 0, behavior: "smooth" });
			}
		}
	...
	let deleteOK =async ()=>{
		...
		setLoading(true);
		let object = {
			url: ApiPath.NGReturnListDelete,
			method: 'delete',
			params: {
				"login_id": loginID,
								...
								<table className='table ng-return-list-table'>
									...
									{userData.map((item, index) => (
												...
												<tr key={index}>
													...
													<td>{item.made_in}</td>
													<td>{dateFormatChange1(item.ng_return_date)}</td>
													<td>{item.price}</td>
													<td>	
														{item.edit_flag == true ?
...
export default NGReturnListIndex
